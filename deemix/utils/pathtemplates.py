...
def generateTrackName(filename, track, settings):
    ...
    filename = filename.replace("%upc%", str(track.album.barcode or ''))
...
def generateAlbumName(foldername, album, settings, playlist=None):
    ...
    foldername = foldername.replace("%type%", fixName(str(album.recordType or '').capitalize(), c))
    foldername = foldername.replace("%upc%", str(album.barcode or ''))
